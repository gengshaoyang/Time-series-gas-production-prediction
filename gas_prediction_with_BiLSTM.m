clc;clear;close all;	
%% Load data
data_str='C:\Users\80500\Desktop\gas production data\Well_A.xlsx' ;  % data path
...
%% Model training	
select_predict_num = 1;  
num_series = 10;   	
min_batchsize = 30; % batchsize
max_epoch_LC = 20; % epoch
list_cell =	{[1]};
...
for NUM_all=1:length(gas_production_cell)	
    ...
    [~,y_feature_label1]=timeseries_process2(gas_production,select_predict_num,num_series);	
    ...
    if(length(hidden_size)<2)	
       ...
       bilstmLayer(hidden_size(1), 'OutputMode', 'sequence')      % LSTM	
       reluLayer                                                  % Relu
    ...
    y_train_predict_cell{1,NUM_all}=y_train_predict_norm.*y_sig+y_mu;  % 反归一化	
...
train_MAPE=sum(sum(abs((y_train_predict-train_y)./train_y)))/size(train_y,1)/size(train_y,2); disp([Tvalue,' Training set MAPE：',num2str(train_MAPE)])	
train_MSE=(sum(sum(((y_train_predict-train_y)).^2))/size(train_y,1)/size(train_y,2)); disp([Tvalue,' Training set MSE：',num2str(train_MSE)])    	
...
vaild_MAPE=sum(sum(abs((y_vaild_predict-vaild_y)./vaild_y)))/size(vaild_y,1)/size(vaild_y,2); disp([Tvalue,' Validation set MAPE：',num2str(vaild_MAPE)])	
vaild_MSE=(sum(sum(((y_vaild_predict-vaild_y)).^2))/size(vaild_y,1)/size(vaild_y,2)); disp([Tvalue,' Validation set MSE：',num2str(vaild_MSE)])     	
...
test_MAPE=sum(sum(abs((y_test_predict-test_y)./test_y)))/size(test_y,1)/size(test_y,2); disp([Tvalue,' Testing set MAPE：',num2str(test_MAPE)])	
test_MSE=(sum(sum(((y_test_predict-test_y)).^2))/size(test_y,1)/size(test_y,2)); disp([Tvalue,' Testing set MSE：',num2str(test_MSE)]) 	
